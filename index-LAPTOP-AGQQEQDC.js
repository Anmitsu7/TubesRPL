    ...
    connection.beginTransaction(async (err) => {
      ...
      try {
        ...
        const [userResult] = await connection.query(
          'INSERT INTO user (namaUser, email, password, tanggalLahir, alamat, nomorTelepon, role) VALUES (?, ?, ?, ?, ?, ?, "pasien")',
          [namaUser, email, hashedPassword, tanggalLahir, alamat, nomorTelepon]
        );

        // 2. Insert booking tanpa nomor antrian
        await connection.query(
          'INSERT INTO booking (pasienId, jadwalId, tanggalBooking, metodePendaftaran, status, nomorAntrian, statusAntrian) VALUES (?, ?, NOW(), "offline", "aktif", NULL, "menunggu")',
          [userResult.insertId, jadwalId]
        );

        // 3. Update sisa kuota
        await connection.query(
          'UPDATE jadwal_dokter SET sisaKuotaOffline = sisaKuotaOffline - 1 WHERE idJadwal = ? AND sisaKuotaOffline > 0',
          [jadwalId]
        );
